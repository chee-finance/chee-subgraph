...
let cMATICAddress = '0x5a46256055de0f74635ff59a07174a9fc9d5a0bf'
...
export function createMarket(marketAddress: string): Market {
  ...
  // It is cMATIC, which has a slightly different interface
  if (marketAddress == cMATICAddress) {
    ...
    market.underlyingName = 'Matic Coin'
...
function getBNBinUSD(blockNumber: i32): BigDecimal {
  ...
  let bnbPriceInUSD = oracle
    .getUnderlyingPrice(Address.fromString(cMATICAddress))
    .toBigDecimal()
...
export function updateMarket(
  ...
  if (market.accrualBlockNumber != blockNumber) {
    ...
    // if cMATIC, we only update USD price
    if (market.id == cMATICAddress) {
      ...
      // if USDC, we only update MATIC price
